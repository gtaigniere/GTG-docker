...
CREATE TABLE IF NOT EXISTS rubric (
    idRub INT NOT NULL AUTO_INCREMENT,
...
--
INSERT INTO rubric (idRub, label, image)
VALUES (NULL, 'Boole', 'algebre_de_boole.jpg'),
...
--
ALTER TABLE link ADD CONSTRAINT ´lien_rubric_fk´ FOREIGN KEY (idRub) REFERENCES rubric (idRub),
...
VALUES (NULL, 'Notions fondamentales', 'Algo-Notions_fondamentales-Cours.pdf', 2, 1),
...
VALUES (NULL, 'gilleste', 'gilleste@nanterre.fr', '$2y$12$YRgfcdmX2X8KZx3.lMQG3OgyoXi3SOe4uTnf31jjSWuPL2X5LYvoG', NULL,
        FALSE),
       (NULL, 'davidht', 'davidht@soisy.fr', '$2y$12$WHCdRWGsVVvZKUPI77uBbOZNk4iavwqpuJZJeorcrx/SO2s1E9wn.', NULL,
        FALSE),
       (NULL, 'didierhn', 'didierhn@paris.fr', '$2y$12$c/oLC8PUBGkGnijJ2A9oLuvRbO/77B5jIsmeCwpmqM4WGLMSy/MW2', NULL,
        FALSE),
       (NULL, 'jérémyht', 'jeremyht@etel.fr', '$2y$12$YAdLTb/ZhTIrcjvH7ygAbuBxvcWvFUxIdrmYC3T9T/no0xjoBpAE.', NULL,
        FALSE);
...
CREATE TABLE snippet (
  ...
  DEFAULT CHARSET = utf8
  COLLATE = utf8_general_ci;
ALTER TABLE snippet ADD CONSTRAINT ´snip_user_fk´ FOREIGN KEY (idUser) REFERENCES user (idUser);
ALTER TABLE snippet ADD CONSTRAINT ´snip_lang_fk´ FOREIGN KEY (idLang) REFERENCES language (idLang);

--
-- Contenu de la table `snippet`
--
INSERT INTO snippet (idSnip, title, code, dateCrea, comment, requirement, idLang, idUser)
VALUES (NULL, 'A vérifier', '<?php echo \'test\'; ?>\r\npeut être remplacé par\r\n<?= \'test\'; ?>\r\nA confirmer',
        '2020-02-05 12:35:27', 'Voir avec Matthieu', NULL, 1, 1),
       (NULL, 'VarLetConst', 'VAR est remplac&eacute; par CONST et LET depuis ES...', '2020-02-07 09:46:52', '', '', 2,
        2),
       ...
       (NULL, 'Envoyer un mail', 'mail("label@fournisseur", "Sujet", $message, $header);', '2020-02-07 00:00:00', NULL,
        NULL, 1, 2),
       (NULL, 'Test ajout', 'var $add = "Ajout d\'un snippet";console.log($add...', '2020-02-09 12:31:40', '', '', 2,
        1),
...
CREATE TABLE snipcat (
  ...
  DEFAULT CHARSET = utf8
  COLLATE = utf8_general_ci;
ALTER TABLE snipcat ADD CONSTRAINT ´snipcat_snippet_fk´ FOREIGN KEY (idSnip) REFERENCES snippet (idSnip);
ALTER TABLE snipcat ADD CONSTRAINT ´snipcat_cat_fk´ FOREIGN KEY (idCat) REFERENCES cat (idCat);
...
--
INSERT INTO contact (id, firstname, mail, object, received, message)
VALUES (NULL, 'Dylan', 'dylan@free.fr', 'Question sur PHP', '2020-06-15 11:32:17',
        'Bonjour, peut on faire un constructeur en PHP ?'),
       (NULL, 'Bruno', 'bruno@bbox.fr', 'Tableaux en JavaScript', '2019-09-25 12:55:47',
        'Bjr, possible de me dire si cette ligne vous parait ok ?'),
       (NULL, 'Maxime', 'Maxime@yahoo.com', 'Python en backend', '2020-07-10 16:52:35',
        'Avez-vous des connaissances en Python ?');

-- Requête avec un AND sur la même colonne
SELECT s.* FROM snippet s
     JOIN snipcat sc ON sc.idSnip = s.idSnip
    WHERE sc.idCat = 2 AND sc.idSnip IN (
